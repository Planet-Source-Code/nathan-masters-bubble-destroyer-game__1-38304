...
Const MINDISTFROMSHIP = 100
Const ENEMYACCRATE = 0.1
...
'Rof
    If KeyCode = vbKey4 Then
        If Not GunStyle(GunMode).Delay <= FDELAY Then
            GunStyle(GunMode).HeatRate = GunStyle(GunMode).HeatRate - 0.15
...
'Heat
    If KeyCode = vbKey5 Then
        If Not GunStyle(GunMode).HeatRate <= 1 Then
            GunStyle(GunMode).HeatRate = GunStyle(GunMode).HeatRate - 0.35
            BonusLevels = BonusLevels - 1
        End If
    End If
'range
...
'size
    If KeyCode = vbKey7 Then
        If Not GunStyle(GunMode).Radius >= 20 Then
...
'velocity
    If KeyCode = vbKey8 Then
        If Not GunStyle(GunMode).Speed >= 10 Then
            GunStyle(GunMode).Speed = GunStyle(GunMode).Speed + 1
            BonusLevels = BonusLevels - 1
        End If
    End If
'veloc Down
...
If GetTickCount - InGameTimer >= FDELAY Then
    ...
    Else
        ...
        End If
        TempFile = FreeFile
        Open App.Path & "/HighScores.rec" For Binary Access Read Write As #TempFile
        Get TempFile, 1, HighScore
        Get TempFile, , RecordHoldersName
        Get TempFile, , DateSet
        Close #TempFile
...
For i = 1 To UBound(Bullet)
    If Bullet(i).Active Then
        ...
        Bullet(i).TimeInFlight = Bullet(i).TimeInFlight + CurDelay
...
For i = 1 To UBound(EnemyBullet)
    If EnemyBullet(i).Active Then
        ...
        Next a
        If EnemyBullet(i).Active Then
        'draw the "shield" circle around the ship
            If CircCol(ShipCenterX, ShipCenterY, SHIPLNG * 2, EnemyBullet(i).x, EnemyBullet(i).y, EnemyBullet(i).Radius) Then
                frmMain.FillColor = vbRed
                MoveToEx MainDC, ShipCenterX, ShipCenterY, TempPoint
                Ellipse MainDC, (ShipCenterX - SHIPLNG * 2), (ShipCenterY - SHIPLNG * 2), (ShipCenterX + SHIPLNG * 2), (ShipCenterY + SHIPLNG * 2)
                frmMain.FillColor = vbWhite
            End If
...
Put SAVEGAMEFILE, , ULTIMATEGUNACCESS
Close
...
Get SAVEGAMEFILE, , ULTIMATEGUNACCESS
frmBack.picLifeBack.Width = MAXLIFE
frmBack.picLifeBack.Height = 20
frmBack.picLifeBar.Width = MAXLIFE
frmBack.picLifeBar.Height = 20
...
Dim Temp As Byte
Dim i As Single
Dim EncryptFile As Integer

Dim EncHighScore As Double
Dim EncRecordHoldersName As String * 20
Dim EncDateSet As Variant

EncryptFile = FreeFile
...
If Not CHEATED Then
    ...
    Get HIGHSCOREFILE, 1, HighScore
    Get HIGHSCOREFILE, , RecordHoldersName
    Get HIGHSCOREFILE, , DateSet
    ...
    If Score > HighScore Then
        ...
        Put HIGHSCOREFILE, 1, Score
        Put HIGHSCOREFILE, , (RecordHoldersName)
        Put HIGHSCOREFILE, , DateSet
